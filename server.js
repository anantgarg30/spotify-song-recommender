...
const { getArtistId, getRecommendations } = require("./spotify/actions");
...
app.post("/recommendations", async (req, res) => {
  ...
  // 2. get artist ids from search
  let artistId1, artistId2, artistId3;
  
  try {
    const result = await getArtistId(http, { artist1 })
  ...
  try {
    const result = await getArtistId(http, { artist2 })
  ...
  try {
    const result = await getArtistId(http, { artist3 })
    const { artists } = result
